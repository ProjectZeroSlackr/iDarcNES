...
int but_mod=0;
int but_mod2=0;
extern unsigned int *kb[7], *menu, *sel, *play,
    *rev, *fwd;
unsigned int joy_up, joy_down, joy_left, joy_right,
    joy_a, joy_b, joy_start, joy_select;
...
void cfg(int k){
    ...
    if (fsprnum>63) fsprnum=0;
		...
		case ROMSMENU:
			...
			break;
		case KEYSMENU:
				if (selected==0) outcfg=(outcfg+1)>2?0:outcfg+1;
				if (selected==1) actcfg=(actcfg+1)>2?0:actcfg+1;
				if (selected==2) 
				{
					diags^=1;
					if (diags) butcfg=butcfgwod; else butcfg=butcfgwd;
				}
				input_updated=1;
	}
...
void doMenu()
{
	...
	switch(whereinmenu)
	{
		case MAINMENU:
			...
			printMenu("ROM LIST","",selected==1);
			printMenu("SAVE","",selected==2);
			break;
		case CFGMENU:
			totalentries=4;		
...
//			for (i=0; i<64; i++)
	...
	if (doRomMenu("/bin")==0) 
		...
		case KEYSMENU:
			totalentries=6;		
			printMenu("OUTER WHL",keycfgtype[outcfg],selected==0);
			printMenu("ACTION",keycfgtype[actcfg],selected==1);
			printMenu("DIAGONALS",diags?"ON":"OFF",selected==2);
			printMenu("UPPER-LEFT",ul,selected==3);
			printMenu("UPPER-RIGHT",ur,selected==4);
			printMenu("LOWER-RIGHT",lr,selected==5);
			printMenu("LOWER-LEFT",ll,selected==6);
			break;
