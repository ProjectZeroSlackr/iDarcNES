...
extern unsigned int *kb[7], *menu, *sel, *play,
    *rev, *fwd;
    extern unsigned char *kbc[7], *menuc, *selc, *playc,
    *revc, *fwdc;
unsigned int joy_up, joy_down, joy_left, joy_right,
    joy_a, joy_b, joy_start, joy_select;
...
void ssave()
{
  FILE*fp=fopen("/mnt/nes/idarcnes.cfg","w");
  int i;
  for(i=0;i<=7;i++)
  fprintf(fp,"kb%i=%c\n",i,kbc[i]);
...
void cfg(int k){
    ...
    if (fsprnum>63) fsprnum=0;
		...
		case ROMSMENU:
			...
			break;
		case KEYSMENU:
				if (selected==0) keyc(&menuc);
				if (selected==1) keyc(&selc);
				if (selected==2) keyc(&playc);
				if (selected==3) keyc(&fwdc);
				if (selected==4) keyc(&revc);
				if (selected==5) keyc(&kbc[0]);
				if (selected==6) keyc(&kbc[1]);
				if (selected==7) keyc(&kbc[2]);
				if (selected==8) keyc(&kbc[3]);
				if (selected==9) keyc(&kbc[4]);
				if (selected==10) keyc(&kbc[5]);
				if (selected==11) keyc(&kbc[6]);
				if (selected==12) keyc(&kbc[7]);
			break;
